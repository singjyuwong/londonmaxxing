...
function toBillFactors({ numPeople, weekdayLocation, heatingFuel, tempTier, heatingScope, showerTier }) {
  const occupants = numPeople >= 3 ? '3+' : numPeople;
  const presence = { home: 'wfh', hybrid: 'hybrid', away: 'office' }[weekdayLocation];
  const roomsHeated = heatingScope === 'rooms' ? 'rooms_i_use' : 'whole_house';
  const showerFrequency = SHOWER_TIERS[showerTier].key === 'frequent' ? 'many' : SHOWER_TIERS[showerTier].key;
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        if (cancelled) return;
        setAddressOptions([]);
        setAddressError(err.message || 'Something went wrong looking up that postcode.');
      } finally {
  ...
  function startAnalysis() {
    ...
    const factors = toBillFactors({ numPeople, weekdayLocation, heatingFuel, tempTier, heatingScope, showerTier });
  ...
  const costs = certificate
    ? [
        { key: 'heating', label: 'Heating Cost', icon: Flame, current: certificate.heating_cost_current, potential: certificate.heating_cost_potential },
        { key: 'hotWater', label: 'Hot Water Cost', icon: Droplet, current: certificate.hot_water_cost_current, potential: certificate.hot_water_cost_potential },
        { key: 'lighting', label: 'Lighting Cost', icon: Lightbulb, current: certificate.lighting_cost_current, potential: certificate.lighting_cost_potential },
      ]
    : [];
  const fabric = certificate
    ? [
        ...(certificate.walls ?? []).map((w) => ({ label: 'Walls', value: w.description })),
        ...(certificate.roofs ?? []).map((r) => ({ label: 'Roof', value: r.description })),
        ...(certificate.floors ?? []).map((f) => ({ label: 'Floors', value: f.description })),
        ...(certificate.main_heating ?? []).map((m) => ({ label: 'Main heating', value: m.description })),
        ...(certificate.main_heating_controls ?? []).map((m) => ({ label: 'Main heating controls', value: m.description })),
      ]
    : [];
          ...
          <section className="fade-in">
            <div className="grid md:grid-cols-2">
              ...
              <div className="p-10 md:p-14 flex flex-col justify-center min-h-[75vh] translate-y-[20%]">
                ...
                {selectedProperty && (
                  <div className="flex items-center gap-2 mt-3 text-sm">
                    <span className="text-slate-500">
                      Selected: <span className="font-semibold text-ink">{formatAddress(selectedProperty)}</span>
                    </span>
                    <button onClick={changeAddress} className="text-orange-accent text-xs font-semibold hover:underline">
                      Change
                    </button>
                  </div>
                )}
                ...
                {certificateError && (
                  <p className="mt-3 text-sm text-red-500">{certificateError}</p>
                )}
                ...
                <div className="flex items-center gap-3 mt-4 flex-wrap">
                  <button
                    ...
                    className={
                      ...
                      (certificate && !certificateLoading ? 'hover:opacity-90' : 'opacity-40 cursor-not-allowed')
                    }
                  >
              ...
              <div className="border-2 border-ink rounded-2xl p-5 mb-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2 shrink-0 ml-3">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-[10px] text-slate-400 mt-1 whitespace-nowrap">Current {certificate.energy_rating_current}/100</span>
                    </div>
                    <ArrowRight className="w-3.5 h-3.5 text-slate-300" />
                    <div className="flex flex-col items-center">
                      <EnergyBand band={certificate.potential_energy_efficiency_band} />
                      <span className="text-[10px] text-slate-400 mt-1 whitespace-nowrap">Potential {certificate.energy_rating_potential}/100</span>
                    </div>
                  </div>
                </div>
                  ...
                  {costs.map((c) => (
                    <div key={c.key} className="bg-slate-50 rounded-lg p-3">
                      ...
                      <div className="text-[10px] text-slate-400 mt-0.5">
                        {c.potential ? `Could be ${formatCurrency(c.potential)}` : ''}
                      </div>
                    </div>
                  ))}
                ...
                <div className="text-sm text-slate-600 mb-4 space-y-1">
                  {certificate.lighting?.description && (
                    <div><span className="font-medium text-ink">Lighting:</span> {certificate.lighting.description}</div>
                  )}
                  {certificate.hot_water?.description && (
                    <div><span className="font-medium text-ink">Hot water:</span> {certificate.hot_water.description}</div>
                  )}
                </div>
                  ...
                  <>
                    ...
                    <div className="space-y-2">
                      {improvements.map((imp) => (
                        <div key={imp.sequence} className="border-2 border-slate-100 rounded-xl p-3 flex items-center justify-between gap-3 text-sm">
                          <div>
                            <span className="font-medium text-ink">{imp.sequence}. {imp.improvement_type}</span>
                            {imp.indicative_cost && <span className="text-slate-400 text-xs"> ({imp.indicative_cost})</span>}
                          </div>
                          {imp.typical_saving && (
            ...
            <div className="max-w-2xl mx-auto px-6 py-10">
              ...
              <div className="flex items-center justify-between mb-1">
                <h2 className="text-2xl font-black text-ink tracking-tight">
                  {formatCertificateAddress(certificate).split(',')[0].toUpperCase()} — BILL ESTIMATE
                </h2>
